...
const ProductsTable = ({ products, errors, onProductChange, onProductBlur, onAddProduct, onRemoveProduct }) => {
  return (
        ...
        <div className='overflow-hidden rounded-2xl border border-slate-800'>
            ...
            <table className='min-w-full divide-y divide-slate-800 text-left'>
                ...
                <thead className='bg-slate-950/60'>
                    <tr>
                        <th className='px-4 py-3 text-sm font-semibold text-slate-200'>Descripcion</th>
                ...
                <tbody className='divide-y divide-slate-800 bg-slate-900 text-sm text-slate-300'>
                ...
                products.map((product) => {
                  ...
                  return (
                    <tr
                        key={product.id}>
                        {/* INPUT DE PRODUCTO DESCRIPCION */}
                        <td className='px-4 py-4'>
                            <input
                                type='text'
                                value={product.description}
                                onChange={(event) => 
                                    onProductChange(product.id, 'description', event.target.value)}
                                ...
                                text-slate-100 placeholder:text-slate-500 focus:outline-none focus:ring-2 focus:ring-emerald-400/50"
                            /> 
                        </td>
