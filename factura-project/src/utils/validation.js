...
//Validar cedula de rd 
//recibe 11 digitos como string y devuelve true si la cedula es valida
function validateDominicanCedula(digits) {
    //pesos alternados 1 y 2 para los primeros 10 digitos
    const weights = [1, 2, 1, 2, 1, 2, 1, 2, 1, 2]
    let sum = 0

    for (let i = 0; i < 10; i++) {
        //Multiplicar cada digito por su peso
        let product = parseInt(digits[i], 10) * weights [i]
        //Si el resultado es mayor a 9, lo reseteamos(equivale a sumar los digitos)
...
//VALIDAR EL CAMPO RNC O CEDULA
//RNC 9 DIGITOS
//CEDULA 11 DIGITOS
export function validateDoc(value) {
    if(!value.trim()) {
        return "El RNC o Cedula no es obligatorio."
    }
